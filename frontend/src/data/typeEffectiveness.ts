...
// Attacker type → defender type → multiplier (0 = immune, 0.5 = not very, 1 = normal, 2 = super)
...
export function getEffectiveness(attackerType: PokemonType, defenderTypes: PokemonType[]): number {
  return defenderTypes.reduce((acc, dt) => {
    const mult = TYPE_CHART[attackerType]?.[dt]
    return acc * (mult === undefined ? 1 : mult)
  }, 1)
}
